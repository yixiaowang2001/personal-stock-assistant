...
async def _call_portfolio_llm(
  ...
  schema_description = {
    ...
    "sector_advice": "字符串，对行业或板块配置方向的建议；仅针对美股市场的行业或领域（例如美股军工、美股科技等），可以点名 1–2 只美股示例股票作为参考，不要出现 A 股或港股代码，这些示例不应出现在 suggestions 数组中",
  ...
  user_prompt = (
    ...
    "4）从行业与现金配置角度进行补充："
    "   - 在 sector_advice 中给出行业或板块配置建议，但请仅针对美股市场的行业和标的（例如美股军工、美股科技等），不要推荐 A 股或港股行业/股票；若当前组合过于集中，可建议分散或增配的美股行业方向，并可以点名 1–2 只美股示例股票作为参考；"
